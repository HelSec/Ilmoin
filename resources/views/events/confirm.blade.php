...
@section('title', 'Cancel your attendance to ' . $event->name)
@section('content')
    <div class="card">
        <div class="flex">
            <div>
                <div class="font-semibold text-2xl mb-2">
                    You're about to confirm your attendance to <a href="{{ route('events.show', $event) }}" class="font-bold hover:underline">{{ $event->name }}</a>
                </div>
    ...
    <div class="card">
        <form action="{{ route('events.confirm', $event) }}" method="post" class="inline-block mr-4">
            @csrf
        ...
        <form action="{{ route('events.cancel', $event) }}" method="post" class="inline-block">
            @csrf
            <button type="submit" class="button-pink">
                Cancel attendance
            </button>
